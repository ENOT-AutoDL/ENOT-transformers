...
def test_acc(
    ...
    dataset_name : str
        Name of the dataset. Default value is "lambda".
    dataset_split : str
        Which split of the data to load. Default value is "test".
    tokenizer_name : str
        Name of the tokenizer. Default value is "gpt2".
    device : Union[str, torch.device]
        Target device for dataset data. Default value is "cuda".
    verbose : bool
    ...
    for batch in pbar:
        input_ids = batch['input_ids'].to(dtype=torch.int32, device=device)
...
def test_latency(
    ...
    if verbose:
        ...
        print()
        print('SEQUENCE_LENGTH  GENERATE_LENGTH  TIME(ms)  TIME_STD(ms)')
